...
verified in `services/front_of_house/tests/orders/test_kitchen_client.py`.
...
| Front-of-house's JWKS endpoint | unreachable from kitchen | Kitchen's `JWKSClient` has no key to verify against → every request 401s. Kitchen keeps cooking whatever's already queued — nothing about the tick loop or slot allocation depends on front-of-house — it just can't accept *new* tickets until the JWKS endpoint is reachable again. |
| A service token | expired mid-flight | Kitchen 401s; front-of-house does **not** retry or count it against the breaker (4xx is a real answer) — surfaces immediately as a rejected order. |
