# 0002 — Phase 2 front-of-house foundations
...
`POST /auth/token` issues HS256-signed JWTs verified only by front-of-house
itself. SPEC.md §6.3 specifies RS256 with a published JWKS so `kitchen` and
`dispatch` can verify front-of-house-issued tokens without a shared secret — but
...
event spine until Phases 3 and 4. `front_of_house.orders.progression` stands in with
...
`services/front_of_house/openapi.json`; `openapi-typescript` generates
...
The storefront (`:5173`) calls front-of-house (`:8000`) cross-origin in dev.
