...
own Postgres, no front-of-house connection string) and wiring it into front-of-house's
order flow without front-of-house ever touching kitchen's database.
...
existed alongside front-of-house that, until Phase 3, drove the entire FSM by
...
### 2. Front-of-house's `Order.status` becomes a passive mirror of kitchen's events, not a re-derivation of them

Before kitchen existed, `front_of_house.orders.tasks` drove `enqueue` through
`mark_ready` itself, so every transition went through `fsm.apply_transition`
directly. Now kitchen drives those transitions and front-of-house only hears about
...
already validated the transition before firing the event, and front-of-house's own
...
Front-of-house's mirror of `status` intentionally coarsens `prepping` into
`queued` until the next event arrives — a real, documented simplification,
not a bug to fix later, because there is no event that would let front-of-house
...
it (kitchen, dispatch) shares the same Redis" back when only front-of-house existed
...
up front-of-house's own remaining fake steps while kitchen kept baking at the
...
`dinner_rush_core.speed`, so front-of-house and kitchen read the identical Redis
...
requested *before* front-of-house decides to accept. Kitchen's quote returns
`{can_accept, queue_depth, projected_wait_s}`; front-of-house computes
...
- Phase 7 (dispatch extraction) inherits the same integration pattern as
  ...
  `courier.assigned`/`order.picked_up`/`order.delivering`, and front-of-house's
...
**Give kitchen a full copy of front-of-house's FSM and re-validate every
transition on the front-of-house side.** Rejected — it would require kitchen to
publish an event for every micro-transition (including `start_prep`, which
the catalogue deliberately has no event for) purely so front-of-house's stricter
table stays satisfiable, adding event traffic with no consumer that needs
it.

**Have front-of-house poll kitchen's `/queue` for order status instead of
