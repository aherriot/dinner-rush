...
`dinner_rush_core` or `front_of_house`/`kitchen` imports and find none, rather than
taking CLAUDE.md's word for it.

Front-of-house's own config model (`dinner_rush_core.config.RootConfig`) grows
...
piece of the simulator's config front-of-house has a legitimate reason to read, and
...
| `oven_down`, `ingredient_shortage` | `actions` — admin API calls (`POST /admin/ovens/{id}/status`, `POST /admin/menu/{sku}/availability`) | No — these are chaos-button features PHASES.md assigns to Phase 10, triggered through a `POST /admin/scenarios/{name}/start` endpoint front-of-house doesn't implement yet |
...
front-of-house's admin scenario endpoint) rather than a generic "not implemented."
...
generated by `datamodel-code-generator` from `services/front_of_house/openapi.json`
...
- Phase 10 (chaos, recorded) needs front-of-house's `POST /admin/scenarios/{name}/start`
  to apply `oven_down`/`ingredient_shortage`'s `actions`. That endpoint is
  front-of-house's responsibility, not the simulator's — the simulator only ever
