"""Service-to-service tokens gateway mints for its own outbound calls.

Distinct from `accounts.views._issue_token` (customer/staff login, verified by
gateway itself via `VERIFYING_KEY`): a service token is minted per outbound
...
from gateway.common.keys import get_kid, get_private_key_pem
