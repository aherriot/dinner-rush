...
from front_of_house.accounts.speed import get_speed
...
from front_of_house.orders.serializers import (
    ...
    def post(self, request: Request) -> Response:
        ...
        with transaction.atomic():
            order = Order.objects.create(
                code=_next_order_code(config.front_of_house.order_code_start),
                customer=customer,
                address=address,
                status="placed",
                subtotal_cents=0,
            ...
            fee = pricing.delivery_fee_cents(subtotal, config.front_of_house)
