...
front_of_house. None of the three change the failure direction that was already
correct before Phase 5: a kitchen that's slow, unreachable, or behind an open
breaker is treated as "at capacity" — refusing an order front-of-house can't confirm
...
class KitchenUnavailableError(Exception):
    ...
    (kitchen's panel empty while orders keep flowing through front-of-house) is the
