"""Front-of-house's RS256 signing keypair (SPEC.md §6.3, ADR 0002 §1, ADR 0005).

Front-of-house is the only signer in the system — its private key never leaves
this process. Generated once and cached on disk so restarts reuse the same key
(kitchen's JWKS cache and any live tokens survive a `docker compose restart
front-of-house`); a fresh clone or a wiped `front-of-house-keys` volume just
generates a new one, which is fine for a project that is never hosted
(CLAUDE.md preamble) — there is no production key to protect or rotate.
"""
...
# services/front_of_house/ — anchored to this file rather than left as a bare
...
# (compose.yaml's `front-of-house-keys` volume) regardless.
